...
import helping, { type Helping } from "#tables/helping"
...
export async function refreshHelpingFooter(topic: discord.ThreadChannel) {
	...
	const bestHelpers = Array.from(helpers).sort((a, b) => {
		const rankA =
			ranks.find((r) => r.id === a.id)?.rank ?? Number.POSITIVE_INFINITY
		const rankB =
			ranks.find((r) => r.id === b.id)?.rank ?? Number.POSITIVE_INFINITY
